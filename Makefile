...
unitTest: $(ASSUME_REQUIRED) $(DOTENV_TARGET)
	docker-compose run $(USER_SETTINGS) --rm lambda serverless_ecs_metrics.unit_test
...
styleTest: $(DOTENV_TARGET)
	docker-compose run $(USER_SETTINGS) --rm pep8 --ignore 'E501,E128' serverless_ecs_metrics/*.py

run: $(DOTENV_TARGET)
	cp -a serverless_ecs_metrics/. $(PACKAGE_DIR)/
	docker-compose run $(USER_SETTINGS) --rm lambda serverless_ecs_metrics.handler
